...
fn runtime_reentrancy_check() {
    ...
    let code_1 = format!(
        r#"
        module 0x{0}::B {{
            public fun foo1() {{ Self::dispatch(); return }}
            public fun foo2() {{ Self::dispatch_c(); return }}
            public fun foo3() {{ Self::dispatch_d(); return }}

            native fun dispatch();
            native fun dispatch_c();
            native fun dispatch_d();
        }}
"#,
        TEST_ADDR.to_hex(),
    );
